// app/_components/grammarian/GrammarianReport.tsx
'use client';
...
function groupBySpeaker<T extends { speaker: { id: string; name: string } }>(entries: T[]) {
  ...
  for (const e of entries) {
    const g = groups.get(e.speaker.id) ?? { name: e.speaker.name, items: [] };
    g.items.push(e);
          ...
          <div className="p-3 rounded-lg bg-purple-50 dark:bg-purple-900/20">
            ...
            {wotdUsages.length > 0 ? (
              ...
              <p className="text-sm mt-1 text-gray-500 dark:text-gray-400 italic">
                No speakers used the Word of the Day.
              </p>
            )}
          </div>
        </section>
        ...
        <section>
          ...
          <div className="space-y-3">
            {improperBySpeaker.map((group) => (
              <div key={group.name} className="p-3 rounded-lg bg-red-50 dark:bg-red-900/20">
                ...
                <ul className="mt-1 space-y-1 text-sm">
                  {group.items.map((entry: ImproperUsageEntry) => (
                    <li key={entry.id} className="break-words">
                      ...
                      {entry.suggestion && (
                        <span className="text-gray-600 dark:text-gray-400"> → {entry.suggestion}</span>
                      )}
                    </li>
                  ))}
                </ul>
        ...
        <section>
          ...
          <div className="space-y-3">
            {outstandingBySpeaker.map((group) => (
              <div key={group.name} className="p-3 rounded-lg bg-emerald-50 dark:bg-emerald-900/20">
                <p className="font-semibold text-gray-900 dark:text-gray-100">{group.name}</p>
                <ul className="mt-1 space-y-1 text-sm">
